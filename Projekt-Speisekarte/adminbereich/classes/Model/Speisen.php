...
class Speisen extends ModelAbstract{
    ...
    public function alleAktiveElemente(): array | false{
        ...
        $result = $db_con -> query("SELECT s.speise_id, s.name as sName, s.beschreibung as sBeschreibung, menge, preis, k.name as kName, k.beschreibung as kBeschreibung, k.typ, e.name as eName, e.kuerzel 
            from {$this->tabelle} s, bz_speise_kategorie bz, kategorie k , einheit e where s.aktiv= 1 and s.speise_id=bz.speise_id and bz.aktiv =1 and k.kategorie_id=bz.kategorie_id and k.aktiv=1 and e.einheit_id=bz.einheit_id order by typ, k.name, s.name, menge;");
